...
data_dir <- "D:/CEU/Data_Analysis_3/da_case_studies/"

# set data dir, load theme and functions

source(paste0(data_dir,"ch00-tech-prep/theme_bg.R"))
source(paste0(data_dir,"ch00-tech-prep/da_helper_functions.R"))
...
data <- read_csv("D:/CEU/Data_Analysis_3/da_data_repo/airbnb/clean/airbnb_london_workfile_adj_book.csv") %>%
...
kable(x = result_1, format = "latex", digits = 3) %>%
  cat(.,file= "D:/CEU/Data_Analysis_3/DA3_Yuri/output/rf_models_turning_choices.tex")
...
kable(x = result_2, format = "latex", digits = 3) %>% 
 cat(.,file= "D:/CEU/Data_Analysis_3/DA3_Yuri/output/rf_models_rmse.tex")
...
kable(x = result_3, format = "latex", booktabs=TRUE, linesep = "",digits = c(0,2,1,2), col.names = c("","RMSE","Mean price","RMSE/price")) %>%
  cat(.,file= "D:/CEU/Data_Analysis_3/DA3_Yuri/output/performance_across_subsamples.tex")
...
gbm_grid <-  expand.grid(interaction.depth = c(1, 5, 10), # complexity of the tree
                         n.trees = (4:10)*80, # number of iterations, i.e. trees
                         shrinkage = 0.01, # learning rate: how quickly the algorithm adapts
